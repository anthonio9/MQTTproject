...
int sctpstr_cli(FILE *fp, int sock_fd, struct sockaddr *to, socklen_t tolen)
{
	...
	while (fgets(sendline, MAXLINE2, fp) != NULL)
	{
		if (sendline[0] != '[')
		{
...
int sctpstr_cli_echoall(FILE *fp, int sock_fd, struct sockaddr *to, socklen_t tolen)
{
	struct sockaddr_in peeraddr;
	struct sctp_sndrcvinfo sri;
